...
describe("DashboardView (streaming)", () => {
	...
	describe("task detail responses", () => {
		...
		it("clears a superseded task's loading state instead of leaving the row stuck loading", async () => {
			// Expanding task A then task B supersedes A's in-flight detail
			// request globally, but A's own response must still clear A's
			// loading state — otherwise re-expanding A shows a forever spinner.
			const detailRequests: Array<{ type: string; requestId: string; taskId: string }> = []
			postMessageMock.mockImplementation((message: { type: string; requestId: string; taskId: string }) => {
				if (message.type === "getDashboardTaskDetail") detailRequests.push(message)
			})

			const { getByTestId, queryByTestId, findByRole } = render(<DashboardView onDone={() => {}} />)
			act(() => {
				setConnectedState({
					tasks: {
						"task-a": {
			...
			fireEvent.click(await findByRole("button", { name: "task-a" }))
			fireEvent.click(await findByRole("button", { name: "task-b" }))

			expect(detailRequests).toHaveLength(2)

			// The response for task A arrives "late" (after B's request
			// superseded it as the latest global request).
			act(() => {
				window.dispatchEvent(
					new MessageEvent("message", {
						data: {
							type: "dashboardTaskDetailResponse",
							requestId: detailRequests[0].requestId,
							dashboardTaskDetail: { title: "A detail" },
						},
					}),
				)
			})

			// Re-expand task A: the stale response must have cleared its
			// loading state and committed its payload for the task.
			fireEvent.click(await findByRole("button", { name: "task-a" }))
			await waitFor(() => expect(getByTestId("task-detail-task-a").textContent).toBe("A detail"))
			expect(queryByTestId("dashboard-task-detail-loading")).toBeFalsy()
		})
	})
