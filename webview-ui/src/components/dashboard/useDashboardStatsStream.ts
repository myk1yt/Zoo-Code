...
export function useDashboardStatsStream(options: UseDashboardStatsStreamOptions): UseDashboardStatsStreamResult {
	...
	// Last sequence acknowledged by the reducer; included in resume messages
	// so the coordinator can replay exactly the deltas the webview missed.
	const ackedSequenceRef = useRef(0)
					...
					if (message.action === "didBecomeVisible") {
						...
						if (subscriptionIdRef.current && !visibleRef.current) {
							...
							vscode.postMessage({
								...
								requestId: subscriptionIdRef.current,
								value: ackedSequenceRef.current,
							})
						}
					}
					break
	...
	// ── Track the acknowledged sequence for resume cursors ──────────────────

	useEffect(() => {
		ackedSequenceRef.current = state.sequence
	}, [state.sequence])
