...
interface DashboardViewProps {
	onDone: () => void
	/**
	 * Whether the dashboard tab is currently visible. When false, the stats
	 * stream is paused at the coordinator (deltas are not delivered) and
	 * resumed with the last acknowledged sequence when shown again.
	 */
	visible?: boolean
}
...
const DashboardView = memo(({ onDone, visible = true }: DashboardViewProps) => {
	...
	// In-flight detail requests keyed by requestId → taskId, plus the latest
	// requestId per taskId. A response always clears loading for its own task,
	// even when superseded, so a stale response can never leave a row stuck
	// loading; only the latest request per task may commit its payload.
	const taskDetailRequestTaskIdRef = useRef<Map<string, string>>(new Map())
	const latestTaskDetailRequestByTaskIdRef = useRef<Map<string, string>>(new Map())
	...
	const resetTaskDetails = useCallback(() => {
		...
		taskDetailRequestTaskIdRef.current.clear()
		latestTaskDetailRequestByTaskIdRef.current.clear()
	}, [])
	...
	const fetchTaskDetail = useCallback((taskId: string) => {
		...
		taskDetailRequestTaskIdRef.current.set(requestId, taskId)
		latestTaskDetailRequestByTaskIdRef.current.set(taskId, requestId)
	...
	useEffect(() => {
		const handleMessage = (e: MessageEvent) => {
			...
			if (message.type === "dashboardTaskDetailResponse") {
				...
				const taskId = taskDetailRequestTaskIdRef.current.get(message.requestId)
				if (!taskId) return
				taskDetailRequestTaskIdRef.current.delete(message.requestId)
				...
				// A superseded response still cleared its row's loading state
				// above, but must not overwrite the detail a newer request
				// committed for the same task.
				if (latestTaskDetailRequestByTaskIdRef.current.get(taskId) !== message.requestId) return
