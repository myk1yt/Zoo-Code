...
- Scene setup belongs in `sceneController.ts` and must use the exported Zoo Code test API plus explicit readiness files. Do not gate screenshots with fixed sleeps.
- Every run uses isolated workspace, user-data, and extensions directories and must clean them up on success and failure.
- Identify the Zoo Code webview by its extension ID and frame ancestry, not generated frame IDs or names.
