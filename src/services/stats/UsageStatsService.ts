...
/** Custom pricing maps older than this are refreshed on demand. */
...
export class UsageStatsService {
	...
	constructor(
		...
		if (providerSettingsManager) {
			...
			void refreshPricing()
			// On-demand TTL cache: serve the cached map while fresh; once it
			// goes stale, trigger a single guarded background refresh instead
			// of an unguarded fixed-interval timer.
			this.customPricingProvider = () => {
				if (!refreshInFlight && Date.now() - cachedAtMs > PRICING_CACHE_TTL_MS) {
					refreshInFlight = refreshPricing().finally(() => {
	...
	/**
	 * Whether the legacy NDJSON → SQLite migration has completed. Projection
	 * reads are only authoritative once this is true; before that, unmigrated
	 * NDJSON segments still hold events the database does not know about.
	 */
	...
	private async doInitialize(): Promise<void> {
		...
		if (this.database._isInitialized()) {
			try {
				...
				// Gate projection reads on migration completion: until this is
				// true, unmigrated NDJSON segments still hold events the
				// database does not know about.
				this.migrationComplete = migration.isComplete()
			} catch (err) {
	...
	async ensureInitialized(): Promise<void> {
		...
		// Initialization was never started (e.g. a handler raced service
		// startup): start it now instead of silently reading empty state.
	...
	private async readEventsForQuery(query: StatsQuery): Promise<UsageEventV1[]> {
		// The SQLite projection is only authoritative once the legacy NDJSON
		// migration has completed; before that, unmigrated segments still hold
		// events the database does not know about, so fall back to the store.
		if (this.database._isInitialized() && this.migrationComplete) {
