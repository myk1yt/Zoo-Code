...
export class UsageStatsDatabase {
	...
	public rebuildRollupsFromEvents(): void {
		// Synchronous driver for migration paths: drains the generator past
		// the inter-batch yield points, preserving fully-atomic behavior.
		const iterator = this.rebuildRollupsGenerator()
		while (!iterator.next().done) {
			// Skip the inter-batch yield points.
		}
	}

	/**
	 * Event-loop-friendly variant of {@link rebuildRollupsFromEvents}. Awaits
	 * a macrotask yield between event batches so a full-history rebuild does
	 * not monopolize the extension host event loop. The rebuild remains one
	 * atomic transaction; abandoning the returned promise mid-rebuild unwinds
	 * the generator so the open transaction is rolled back.
	 */
	public async rebuildRollupsFromEventsAsync(): Promise<void> {
		const iterator = this.rebuildRollupsGenerator()
		try {
			while (true) {
			...
			// Unwind the suspended generator: the return-completion thrown at
			// the yield point lands in the body's catch, which rolls back.
			iterator.return()
			throw err
		}
	}
	...
	/**
	 * Shared rebuild body. Yields an event-loop yield point after each event
	 * batch; drivers decide whether to honor it (async driver) or drain past
	 * it (sync driver, preserving historical atomic behavior).
	 */
	private *rebuildRollupsGenerator(): Generator<Promise<void>, void, void> {
		const db = this.getDb()
		...
		try {
			...
			while (true) {
				...
				// Yield point between batches: the async driver awaits it so the
				// event loop breathes during a full-history rebuild; the sync
				// driver drains past it.
