...
function getTotalTokens(event: UsageEventV1): number {
	// Always derive from input+output, aligned with UsageAggregator: stored
	// totalTokens is not authoritative (providers disagree on whether it
	// includes cache/reasoning tokens, and it may be absent).
