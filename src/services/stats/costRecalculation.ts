...
/**
 * Per-provider model-ID lists sorted longest-first, cached on first use.
 * The sort order makes substring matching prefer the most specific known ID;
 * caching avoids re-sorting the whole registry on every lookup.
 */
const sortedRegistryIdsCache = new Map<string, readonly string[]>()

function getSortedRegistryIds(provider: string, registry: Record<string, ModelInfo>): readonly string[] {
	let sorted = sortedRegistryIdsCache.get(provider)
	if (!sorted) {
		sorted = Object.freeze(Object.keys(registry).sort((a, b) => b.length - a.length))
		sortedRegistryIdsCache.set(provider, sorted)
	}
	return sorted
}
...
export function lookupModelInfo(
	...
	if (customPricing) {
		...
		if (cp) {
			// Contract: only the four pricing fields are read downstream (by
			// calculateApiCostAnthropic/OpenAI), so a partial pricing object
			// satisfies the ModelInfo surface for this call. The bare cast
			// avoids synthesizing the full registry shape for custom models.
			return {
	...
	if (registry) {
		...
		for (const knownId of getSortedRegistryIds(provider, registry)) {
	...
	if (modelPricing) {
		// Contract: same as the customPricing branch above — only the four
		// pricing fields are read downstream, so the partial object is a
		// sufficient ModelInfo for the cost calculators.
		return {
 ...
 * and positive, or the computed cost if missing (or recorded as 0, which is
 * not authoritative for custom models that predate query-time pricing).
 *
...
export function getEffectiveCost(event: UsageEventV1, customPricing?: CustomModelPricingMap): number {
	// A stored cost of 0 must not short-circuit recomputation: rollup-era
	// events for custom models recorded 0 before query-time pricing existed.
	if (event.usage.costUsd !== undefined && event.usage.costUsd.value > 0) {
		return event.usage.costUsd.value
	}
