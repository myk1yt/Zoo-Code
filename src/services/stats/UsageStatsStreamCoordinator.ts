...
interface SubscriptionState {
	...
	/**
	 * Deltas were dropped because the sink was hidden while the subscription
	 * stayed active; the next delivery must be a full snapshot to re-anchor
	 * the subscriber's local state.
	 */
	deltaLostWhileHidden: boolean
}
...
export class UsageStatsStreamCoordinator {
	...
	resume(sink: StatsStreamSink, lastSequence: number): void {
		...
		// A negative gap means the acked cursor is ahead of the store (e.g. the
		// store was reset while the subscriber was hidden). Clamp to the store
		// cursor: a future cursor would otherwise suppress delta delivery.
		...
		if (gap > MAX_BATCH_EVENTS) {
			...
			// Small gap (or none) — clamp the cursor and schedule a drain
			state.lastSequence = Math.min(lastSequence, currentLastSeq)
			this.scheduleDrain()
		}
	}
	...
	private drain(): void {
		...
		try {
			...
			for (const sub of activeSubs) {
				...
				// Deltas were dropped while this sink was hidden, so its local
				// state diverged in ways replayed deltas may not recover (e.g.
				// trimmed history). Re-anchor with a full snapshot instead of
				// compounding deltas on top of a stale base.
				if (sub.deltaLostWhileHidden) {
					sub.deltaLostWhileHidden = false
				...
				// Resolve the query time range once per drain: applying events
				// per subscription would otherwise re-resolve (and re-consult
				// Intl formatters) for every single event.
				const timeRange = resolveTimeRange(sub.subscription.range)
	...
	private sendSnapshot(state: SubscriptionState): void {
		...
		try {
			...
			const snapshot = this.assembleSnapshot(state, generation, sequence, recordingPaused, customPricing)

			state.generation = generation
			state.lastSequence = sequence
			state.snapshotSent = true
			state.deltaLostWhileHidden = false
	...
	/**
	 * Assembles the full stream snapshot (rollup stats, session/task page, and
	 * heatmap) for one subscription. Shared by the initial snapshot send and the
	 * post-rebuild refresh. Task-catalog subscriptions also refresh the
	 * subscriber's `visibleTaskIds` from the assembled page.
	 */
		...
		setImmediate(() => {
			void (async () => {
				try {
					...
					// Yield to the event loop between event batches so a
					// full-history rebuild does not block the extension host.
					await this.database.rebuildRollupsFromEventsAsync()
					this.rollupsRebuilt = true
					...
					for (const state of this.subscriptions.values()) {
						...
						try {
							...
							const updatedSnapshot = this.assembleSnapshot(
								state,
								generation,
								sequence,
								recordingPaused,
	...
	private postMessage(state: SubscriptionState, message: ExtensionMessage): void {
		...
		// Skipped deltas advance the host cursor but never reach the
		// subscriber, so flag the subscription for a re-anchoring snapshot on
		// the next delivery instead of silently losing increments.
		if (!state.sink.isVisible() && message.type === "dashboardStatsStreamDelta") {
			state.deltaLostWhileHidden = true
			return
		}
