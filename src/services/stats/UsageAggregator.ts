...
/** Exact length of a calendar day in UTC milliseconds for preset range arithmetic. */
...
/**
 * Intl.DateTimeFormat construction is expensive and these helpers run on the
 * per-event hot path, so formatters are cached per timezone.
 */
...
function getTimezoneOffsetMinutes(date: Date, timezone: string): number {
	...
	const tzFormatter = getWallClockFormatter(timezone)
 ...
 *
 * Preset branches use pure UTC millisecond arithmetic: `from`/`to` are UTC
 * instants of timezone midnights, so day shifting is exact and identical in
 * every timezone (matching statsQueryRange.resolvePresetRange).
 */
export function resolveTimeRange(query: StatsQuery): { from?: Date; to?: Date } {
	if (query.preset) {
		...
		switch (query.preset) {
			...
			case "7d": {
				const to = new Date(startOfDayInTimezone(tzNow, query.timezone).getTime() + DAY_MS)
 ...
 * @param customPricing Optional query-time pricing map for custom models
 * @param timeRange Optional pre-resolved range (see {@link resolveTimeRange}).
 *   Callers evaluating many events for one query should resolve the range
 *   once and pass it, rather than recomputing it per event.
...
export function computeEventContribution(
	...
	const { from, to } = timeRange ?? resolveTimeRange(query)
