...
export class UsageStatsMigration {
	...
	migrate(): { totalMigrated: number; totalSkipped: number; complete: boolean } {
		...
		// Build parent map for root task resolution AND parse each segment in
		// one pass: every segment is read and JSON-parsed exactly once (the
		// parsed events are retained, line-aligned, for the migration loop).
		const { parentMap, parsedSegments } = this.parseSegments(segmentFiles)
		...
		for (let segIdx = startSegmentIdx; segIdx < segmentFiles.length; segIdx++) {
			...
			const parsedSegment = parsedSegments[segIdx]

			if (!parsedSegment.events) {
				throw new StatsMigrationError(
					"STATS_MIGRATION/read/001",
					`Failed to read segment ${segmentFile}`,
					parsedSegment.error,
				)
			}

			const events = parsedSegment.events
			...
			for (let i = startLineIdx; i < events.length; i++) {
				const parsedEvent = events[i]
				if (!parsedEvent) {
					// Blank or quarantined line — keep line alignment for checkpoints.
					continue
				}

				let event: UsageEventV1 = parsedEvent
	...
	/**
	 * Reads and parses every segment exactly once, returning:
	 * - `parentMap`: taskId → parentTaskId collected from ALL parsed lines
	 *   (including schema-invalid ones, which still contribute parent links
	 *   for root resolution), and
	 * - `parsedSegments`: per-segment, line-aligned parsed events (holes at
	 *   blank/quarantined lines) so checkpoint line numbers stay valid.
	 *
	 * A segment that cannot be read yields a `null` event list; the migration
	 * loop turns that into the same read error the old two-pass flow threw.
	 */
	private parseSegments(segmentFiles: string[]): {
		parentMap: Map<string, string | undefined>
		parsedSegments: Array<{
			segmentFile: string
			events: Array<UsageEventV1 | undefined> | null
			error?: unknown
		}>
	} {
		...
		for (const segmentFile of segmentFiles) {
			...
			try {
				...
				parsedSegments.push({ segmentFile, events: null, error: err })
				continue
			}
			...
			const events: Array<UsageEventV1 | undefined> = new Array(lines.length)
			for (let i = 0; i < lines.length; i++) {
				const line = lines[i]
				if (!line.trim()) continue
				try {
					const parsed = JSON.parse(line)
					if (parsed && typeof parsed.taskId === "string" && !parentMap.has(parsed.taskId)) {
					...
					events[i] = result.data
				} catch {
					// Skip unparseable lines
				}
			}

			parsedSegments.push({ segmentFile, events })
		}

		return { parentMap, parsedSegments }
	}
