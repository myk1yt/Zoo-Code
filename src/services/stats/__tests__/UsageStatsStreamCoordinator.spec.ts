...
describe("UsageStatsStreamCoordinator", () => {
	...
	describe("hidden resume after long period", () => {
		...
		it("clamps a future acked cursor so delta delivery is not suppressed", () => {
			const coordinator = new UsageStatsStreamCoordinator(db)
			const sink = new MockSink()
			coordinator.subscribe(sink, makeSubscription())
			sink.messages.length = 0

			// Resume with a cursor AHEAD of the store (e.g. the store was
			// reset while the subscriber was hidden). The clamp must not
			// leave a future cursor that suppresses future delta delivery.
			coordinator.resume(sink, db.getLastSequence() + 50)
			vi.advanceTimersByTime(100)
	...
	describe("visibility-caused delta loss", () => {
		it("sends a snapshot instead of compounding deltas after deltas were dropped while hidden", () => {
			const coordinator = new UsageStatsStreamCoordinator(db)
			const sink = new MockSink()
			coordinator.subscribe(sink, makeSubscription())
			sink.messages.length = 0

			// The sink hides WITHOUT pausing: deltas are dropped but the
			// host cursor advances past them.
			...
			// Visible again: the next delivery must re-anchor local state
			// with a full snapshot rather than deltas on a stale base.
	...
	describe("multiple subscribers", () => {
		...
		describe("auto-rebuild stale rollups", () => {
			...
			it("should auto-rebuild when events exist but derived tables are empty", async () => {
				...
				const rebuildSpy = vi.spyOn(db, "rebuildRollupsFromEventsAsync")
				...
				// Flush the setImmediate chain to run the async rebuild (the
				// rebuild yields between batches, so flush asynchronously).
			...
			it("should send original snapshot when the async rebuild fails", () => {
				// Append an event, then clear derived tables
				db.append(makeEvent({ occurredAt: "2026-07-30T10:00:00Z" }))
				clearDerivedTables()

				// Mock rebuild to throw
				const rebuildSpy = vi.spyOn(db, "rebuildRollupsFromEventsAsync").mockImplementation(() => {
	...
	describe("diff coverage: coordinator edge cases", () => {
		...
		it("sends a post-rebuild task snapshot when a task catalog is configured", async () => {
			const { catalog } = createTaskCatalog([makeHistoryItem({ id: "history-task", ts: 100 })])
			db.append(makeEvent({ occurredAt: "2026-07-30T10:00:00Z" }))
			const rawDb = db as unknown as { db: { exec: (sql: string) => void } }
			rawDb.db.exec("DELETE FROM stats_rollup")
		...
		it("logs when sending a post-rebuild snapshot throws", async () => {
			db.append(makeEvent({ occurredAt: "2026-07-30T10:00:00Z" }))
			const rawDb = db as unknown as { db: { exec: (sql: string) => void } }
			rawDb.db.exec("DELETE FROM stats_rollup")
			rawDb.db.exec("DELETE FROM session_metadata")
