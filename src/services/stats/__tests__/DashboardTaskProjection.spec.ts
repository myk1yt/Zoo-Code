...
describe("DashboardTaskProjection", () => {
	...
	it("derives detail totalTokens from input+output instead of trusting stored totalTokens", () => {
		const catalog = createCatalog([makeHistoryItem({ id: "root", ts: 300, task: "Root" })])
		// The stored totalTokens (999) disagrees with input+output (10+5):
		// stored totals are not authoritative, matching UsageAggregator.
		const reader = createUsageReader(new Map(), [
			{
				...makeEvent({
					taskId: "root",
		...
		const detail = computeTaskDetail(catalog, reader, "root", "request-6")
