...
describe("UsageStatsService", () => {
	...
	describe("diff coverage: initialize error/fallback branches", () => {
		...
		it("ensureInitialized starts initialization when none is in flight", async () => {
			const svc = new UsageStatsService(tempDir)

			await svc.ensureInitialized()

			// Initialization actually ran, not a silent no-op.
			expect(svc.getCoordinator()).not.toBeNull()
			svc.dispose()
		})
	})
	...
	describe("custom pricing refresh and coordinator notification", () => {
		...
		it("refreshes stale pricing on demand without a fixed interval", async () => {
			const mockManager = {
				listConfig: vi.fn().mockResolvedValue([{ name: "profile1", apiProvider: providerIdentifiers.openai }]),
				getProfile: vi.fn().mockResolvedValue({
					openAiModelId: "custom-gpt",
			...
			const refreshSpy = vi.spyOn(mockManager, "getProfile")
			...
			const provider = customService["customPricingProvider"]
			expect(provider?.()).toBeDefined()
			const callsAfterInit = refreshSpy.mock.calls.length
			expect(callsAfterInit).toBeGreaterThan(0)

			// While the cache is fresh, reads do not trigger more refreshes.
			expect(provider?.()).toBeDefined()
			expect(refreshSpy.mock.calls.length).toBe(callsAfterInit)

			// Once the TTL lapses, the next read triggers exactly one
			// overlapping-guarded background refresh (no setInterval).
			const nowSpy = vi.spyOn(Date, "now").mockReturnValue(Date.now() + 11_000)
			try {
				expect(provider?.()).toBeDefined()
				await vi.waitFor(() => expect(refreshSpy.mock.calls.length).toBeGreaterThan(callsAfterInit))
			} finally {
				nowSpy.mockRestore()
			}
