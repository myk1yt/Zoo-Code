...
describe("statsQueryRange", () => {
	describe("resolveStatsQueryRangeMs", () => {
		...
		it("drops unparseable explicit bounds instead of producing NaN", () => {
			// Programmatically constructed queries can bypass schema
			// validation; unparseable bounds must become unbounded, not NaN
			// (NaN would silently exclude every event).
			const range = resolveStatsQueryRangeMs(
				makeQuery({ from: "not-a-date", to: "2026-07-31T23:59:59.999Z" }),
				now,
			)
			expect(range.fromMs).toBeUndefined()
			expect(range.toMs).toBe(Date.parse("2026-07-31T23:59:59.999Z"))
			expect(Number.isNaN(range.fromMs)).toBe(false)
			expect(Number.isNaN(range.toMs)).toBe(false)
		})
	})
