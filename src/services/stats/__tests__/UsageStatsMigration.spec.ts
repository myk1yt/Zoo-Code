...
describe("UsageStatsMigration", () => {
	...
	describe("diff coverage: error branches", () => {
		...
		it("skips unreadable segments when parsing for the parent map", () => {
			...
			const { parentMap, parsedSegments } = (
				migration as unknown as {
					parseSegments(segmentFiles: string[]): {
						parentMap: Map<string, string | undefined>
						parsedSegments: Array<{ segmentFile: string; events: unknown[] | null }>
					}
				}
			).parseSegments(["events-000001.ndjson", "events-000002.ndjson"])

			expect(parentMap.has("task-A")).toBe(true)
			expect(parentMap.get("task-A")).toBe("task-B")
			expect(parentMap.has("task-B")).toBe(false)
			expect(parsedSegments[0].events).not.toBeNull()
			expect(parsedSegments[1].events).toBeNull()
		})
	})
})
