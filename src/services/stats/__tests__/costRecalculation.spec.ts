...
describe("costRecalculation", () => {
	...
	describe("getEffectiveCost", () => {
		...
		it("should recompute when the stored costUsd value is 0", () => {
			// A stored 0 is not authoritative: custom models recorded 0 before
			// query-time pricing existed. Pricing-resolvable events must be
			// recomputed instead of returning the stored zero.
			const event = makeEvent({
				provider: providerIdentifiers.anthropic,
				model: "claude-sonnet-4-5",
				usage: {
