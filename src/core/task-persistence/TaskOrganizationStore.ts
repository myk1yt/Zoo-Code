...
export class TaskOrganizationStore {
	...
	async mutate(
		...
		return this.withLock(async () => {
			const requestId = (mutation as unknown as TaskOrganizationMutationRequestV1).requestId ?? ""
	...
	private targetsEqual(a: TaskOrganizationTargetV1, b: TaskOrganizationTargetV1): boolean {
		...
		switch (a.kind) {
			case "task":
				return a.taskId === (b as Extract<TaskOrganizationTargetV1, { kind: "task" }>).taskId
			case "autoGroup":
				return a.rootTaskId === (b as Extract<TaskOrganizationTargetV1, { kind: "autoGroup" }>).rootTaskId
			case "folder":
				return a.folderId === (b as Extract<TaskOrganizationTargetV1, { kind: "folder" }>).folderId
			default:
				return false
		}
	}
