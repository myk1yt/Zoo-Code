...
import type { UsageStatsStreamCoordinator } from "../../../services/stats/UsageStatsStreamCoordinator"
...
describe("usageStatsMessageHandler", () => {
	...
	describe("handleGetDashboardSessions", () => {
		...
		it("resolves session roots through intermediate tasks with no events using the task catalog", async () => {
			// task-mid produced no usage events, so the event-derived parent
			// map cannot connect task-leaf to task-root without the
			// History-derived ancestor chains from the task catalog.
			const events: UsageEventV1[] = [
				makeEvent({
					taskId: "task-root",
					occurredAt: "2026-07-19T10:00:00.000Z",
				...
				makeEvent({
					taskId: "task-leaf",
					occurredAt: "2026-07-19T11:00:00.000Z",
					parentTaskId: "task-mid",
				}),
			]

			const exportData: JsonExport = { ...mockJsonExport, events }
			const exportStats = vi.fn().mockResolvedValue(exportData)
			const taskCatalog = new DashboardTaskCatalog({
				getAll: () => [
					makeHistoryItem({ id: "task-root", parentTaskId: undefined }),
					makeHistoryItem({ id: "task-mid", parentTaskId: "task-root" }),
					makeHistoryItem({ id: "task-leaf", parentTaskId: "task-mid" }),
				],
			})
			const provider = createMockProvider({ exportStats, getTaskCatalog: () => taskCatalog })

			const message: WebviewMessage = {
				type: "getDashboardSessions",
				requestId: "req-sessions-gap",
				usageStatsQuery: validQuery,
			}
			...
			expect(response?.[0].dashboardSessions).toHaveLength(1)
			expect(response?.[0].dashboardSessions?.[0].taskId).toBe("task-root")
			expect(response?.[0].dashboardSessions?.[0].callCount).toBe(2)
		})
	...
	describe("handleSubscribeDashboardStats", () => {
		...
		it("posts stream error when coordinator is unavailable", async () => {
			const provider = createMockProvider({ getCoordinator: () => null })
		...
		it("posts stream error for invalid subscription payload", async () => {
			...
			const message: WebviewMessage = {
				type: "subscribeDashboardStats",
				requestId: "sub-4",
				dashboardStatsSubscription: { requestId: "sub-4" } as DashboardStatsSubscription, // missing range, sessionPageSize, heatmapRangeDays
			}
	...
	describe("handleReplaceDashboardStatsSubscription", () => {
		...
		it("posts error for invalid payload", async () => {
			...
			const message: WebviewMessage = {
				type: "replaceDashboardStatsSubscription",
				requestId: "replace-2",
				dashboardStatsSubscription: {} as DashboardStatsSubscription,
			}
	...
	describe("handleGetDashboardTaskDetail", () => {
		...
		it("resolves the detail range from the provider's active stream subscription", async () => {
			...
			const coordinator = asDouble<UsageStatsStreamCoordinator>({ getSubscription: vi.fn(() => subscription) })
			const provider = createMockProvider({
				getDatabase: () => mockDb,
				getTaskCatalog: () => taskCatalog,
				getCoordinator: () => coordinator,
			})
			const streamSink: StatsStreamSink = { postMessage: vi.fn(), isVisible: () => true }
			provider.setStatsStreamSink(streamSink)
			...
			expect(coordinator.getSubscription).toHaveBeenCalledWith(streamSink)
			expect(mockDb.queryEventsByTaskIds).toHaveBeenCalledWith(["root", "child"], {
