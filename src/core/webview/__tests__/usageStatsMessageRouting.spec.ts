...
import type { WebviewMessage, StatsQuery, StatsSnapshot, DashboardStatsSubscription } from "@roo-code/types"
import type { ClineProvider } from "../ClineProvider"
import type { UsageStatsService, JsonExport, StatsStreamSink } from "../../../services/stats"
import { UsageStatsService as UsageStatsServiceImpl, UsageStatsStreamCoordinator } from "../../../services/stats"
import type { UsageStatsDatabase } from "../../../services/stats/UsageStatsDatabase"
import type { DashboardTaskCatalog } from "../../../services/stats/DashboardTaskCatalog"

/**
 * Structural bridge for test doubles of classes with private fields
 * (UsageStatsDatabase, UsageStatsStreamCoordinator, DashboardTaskCatalog):
 * one deliberate `unknown → T` assertion per double instead of per-site `as any`.
 */
function asDouble<T>(double: unknown): T {
	return double as T
}
...
const createMockCoordinator = (): UsageStatsStreamCoordinator =>
	asDouble({
	subscribe: vi.fn(),
	unsubscribe: vi.fn(),
...
const createMockDatabase = (): UsageStatsDatabase =>
	asDouble({
...
const createMockProvider = (service?: Partial<UsageStatsService>): ClineProvider => {
	...
	// Backing store for the provider's stream sink accessors, so streaming
	// handlers can persist a sink across messages exactly like the real provider.
	let mockStatsStreamSink: StatsStreamSink | undefined

	return {
		log: mockLog,
		postMessageToWebview: mockPostMessageToWebview,
		getUsageStatsService: vi.fn(() => mockService),
		getStatsStreamSink: vi.fn(() => mockStatsStreamSink),
		setStatsStreamSink: vi.fn((sink: StatsStreamSink | undefined) => {
			mockStatsStreamSink = sink
		}),
...
describe("usageStatsMessageRouting", () => {
	...
	describe("existing usage-stat routing", () => {
		...
		it("routes getDashboardTaskDetail without replacing the legacy session route", async () => {
			...
			const taskCatalog = asDouble<DashboardTaskCatalog>({
	...
	describe("dashboard stream routing", () => {
		...
		it("routes subscribeDashboardStats to handleSubscribeDashboardStats", async () => {
			...
			const provider = createMockProvider({ getCoordinator: () => coordinator })

			const message: WebviewMessage = {
				type: "subscribeDashboardStats",
				requestId: "sub-route-1",
				dashboardStatsSubscription: validSubscription,
			}
		...
		it("routes replaceDashboardStatsSubscription to handleReplaceDashboardStatsSubscription", async () => {
			...
			const message: WebviewMessage = {
				...
				dashboardStatsSubscription: { ...validSubscription, requestId: "replace-route-1" },
			}
		...
		it("routes resyncDashboardStats to handleResyncDashboardStats", async () => {
			...
			const message: WebviewMessage = {
				...
				dashboardStatsSubscription: { ...validSubscription, requestId: "resync-route-1" },
			}
	...
	describe("coordinator disposal", () => {
		it("UsageStatsService.dispose() releases the coordinator and database", async () => {
			// Exercise the real disposal chain (ClineProvider.dispose() calls
			// usageStatsService.dispose()) instead of reimplementing it in a
			// local literal that could drift from the service under test.
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "usage-stats-routing-"))
			try {
				const service = new UsageStatsServiceImpl(tempDir)
				const coordinator = createMockCoordinator()
				// The coordinator is only created by initialize(); inject the
				// double directly. Double assertion: the mock coordinator only
				// implements the narrow surface dispose() touches.
				service["coordinator"] = coordinator as unknown as InstanceType<typeof UsageStatsStreamCoordinator>
				const databaseClose = vi.spyOn(service["database"], "close")

				service.dispose()

				expect(coordinator.dispose).toHaveBeenCalledTimes(1)
				expect(service["coordinator"]).toBeNull()
	...
	describe("request validation and response correlation", () => {
		it("subscribeDashboardStats with invalid payload posts stream error with matching requestId", async () => {
			...
			const message: WebviewMessage = {
				...
				dashboardStatsSubscription: { requestId: "validation-1" } as DashboardStatsSubscription, // missing required fields
			}
