 ...
 *
 * Gap fallback: the event-derived map only contains tasks that produced
 * usage events. When the walk stops at a task with no events of its own
 * (an intermediate subtask), its real root can only be higher in the chain;
 * resolve it from the task catalog's ancestor map (History-derived).
 *
 * @param event The usage event to resolve.
 * @param parentMap Map of taskId → parentTaskId (built from the event set).
 * @param ancestorsByTaskId Optional History-derived ancestor chains
 *   (nearest first, root last) used to bridge event-map gaps.
...
function resolveRootTaskId(
	...
	// Gap fallback: `current` has no usage events, so the event map cannot say
	// whether it is a true root or an intermediate task. Only consult history
	// in that case; a task with an event and no parent IS the root.
	if (!parentMap.has(current)) {
		const ancestors = ancestorsByTaskId?.get(current)
 ...
 * @param ancestorsByTaskId Optional History-derived ancestor chains used to
 *   resolve session roots when the event-derived parent map has gaps.
 */
async function buildSessionSummaries(
	...
	for (const event of events) {
		const rootTaskId = resolveRootTaskId(event, parentMap, ancestorsByTaskId)
	...
	// Session titles are independent per group; derive them in parallel
	// rather than serializing one disk read per session.
	const titles = new Map<string, string>()
	await Promise.all(
		[...groups.keys()].map(async (taskId) => {
			titles.set(taskId, await deriveSessionTitle(taskId, globalStoragePath))
		}),
	)
...
export async function handleGetDashboardSessions(provider: ClineProvider, message: WebviewMessage): Promise<void> {
	...
	try {
		...
		const customPricing = await buildCustomPricingMapFromAllProfiles(provider.providerSettingsManager)
		// History-derived ancestor chains bridge gaps in the event-derived
		// parent map (intermediate tasks that produced no usage events).
		const ancestorsByTaskId = service.getTaskCatalog?.()?.ancestorsByTaskId
		let summaries = await buildSessionSummaries(events, globalStoragePath, query.cacheRatio, customPricing, ancestorsByTaskId)
...
export async function handleGetDashboardSessionDetail(provider: ClineProvider, message: WebviewMessage): Promise<void> {
	...
	try {
		...
		// from all subtasks whose root resolves to this taskId. History-derived
		// ancestor chains bridge gaps where intermediate tasks have no events.
		const parentMap = buildParentMap(allEvents)
		const ancestorsByTaskId = service.getTaskCatalog?.()?.ancestorsByTaskId
		const taskEvents = allEvents.filter((ev) => resolveRootTaskId(ev, parentMap, ancestorsByTaskId) === taskId)
...
function resolveTaskRangeMs(provider: ClineProvider, service: UsageStatsService | undefined): StatsQueryRangeMs {
	...
	const sink = provider.getStatsStreamSink()
 ...
 * The sink is stored on the provider via {@link ClineProvider.setStatsStreamSink}
 * so it persists across messages but is cleaned up when the provider is disposed.
 *
...
async function getCoordinatorAndSink(
...
): Promise<{ coordinator: UsageStatsStreamCoordinator; sink: StatsStreamSink } | null> {
	const service = provider.getUsageStatsService()
	...
	let sink = provider.getStatsStreamSink()

	if (!sink) {
		sink = new ProviderStreamSink(provider)
		provider.setStatsStreamSink(sink)
	}
