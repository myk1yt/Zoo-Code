...
describe("Usage Stats Recording", () => {
	...
	describe("Terminal finalize boundary", () => {
		...
		it("emits a terminal usage event when a request completes with zero usage tokens", async () => {
			const task = new Task({
				provider: mockProvider,
				apiConfiguration: mockApiConfig,
				task: "zero usage task",
				startTask: false,
			})

			const recorder = (task as unknown as Record<string, unknown>).usageRecorder as UsageRecorder
			const finalizeSpy = vi.spyOn(recorder, "finalizeUsageEvent").mockResolvedValue(undefined)

			// Avoid model-fetch network access inside the request loop.
			vi.spyOn(task as unknown as TaskTestAccess, "safeEnsureModelFetched").mockResolvedValue(undefined)
			vi.spyOn(task.diffViewProvider, "reset").mockResolvedValue(undefined as never)

			// The main loop breaks after the first chunk (abort), leaving the
			// trailing zero-value usage chunk for the background drain to
			// collect. Token totals stay zero for the entire request.
			const zeroUsageStream = (async function* (): ApiStream {
				yield { type: "text", text: "partial" }
				;(task as unknown as Record<string, unknown>).abort = true
				yield { type: "usage", inputTokens: 0, outputTokens: 0, cacheWriteTokens: 0, cacheReadTokens: 0, totalCost: 0 }
			})()
			vi.spyOn(task, "attemptApiRequest").mockReturnValue(zeroUsageStream)

			const result = await task.recursivelyMakeClineRequests([{ type: "text", text: "hello" }], false)
			expect(result).toBe(true)

			// The drain runs in the background; wait for the terminal finalize.
			await vi.waitFor(() => expect(finalizeSpy).toHaveBeenCalledOnce())
			const [, status, ctx] = finalizeSpy.mock.calls[0] as [string, string, UsageRecordingContext]
			expect(status).toBe("cancelled")
			expect(ctx.inputTokens).toBe(0)
			expect(ctx.outputTokens).toBe(0)
			expect(ctx.cacheWriteTokens).toBe(0)
