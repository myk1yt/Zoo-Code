 ...
 * is not a string. Empty strings, missing values, and null are accepted
 * (the downstream tool treats them as "use workspace default"). The parser
 * normalizes null → undefined before validation, but this validator also
 * accepts null as a defense-in-depth measure.
 *
...
export function validateCwdParameter(args: Record<string, unknown>, toolName?: string): InterceptionSignal | null {
	...
	const cwd = args.cwd
	// Accept undefined (omitted), null (normalized to undefined by parser),
	// and string values. Reject objects, arrays, numbers, and booleans.
